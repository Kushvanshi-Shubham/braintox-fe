...
const CardComponent = ({ content, refresh, collectionId, onContentRemoved, selectionMode, selected, onToggleSelect }: CardProps) => {
  ...
  const [isPrivate, setIsPrivate] = useState(content.isPrivate || false);
  ...
  const togglePrivacy = async () => {
    try {
      ...
      await axios.patch(
        `${BACKEND_URL}/api/v1/content/${contentId}/privacy`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      <div className="relative p-3 sm:p-5">
        ...
        <div className="mb-3 sm:mb-4">
          <div className="flex items-center gap-1.5 sm:gap-2 mb-2 sm:mb-3">
            ...
            {isPrivate && (
              <span className="flex items-center gap-1 text-[10px] sm:text-xs font-semibold px-2 py-0.5 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-500 dark:text-gray-400 border border-gray-200/50 dark:border-gray-600/50" title="Only you can see this">
                🔒 Private
              </span>
            )}
          </div>
          ...
          <div className="flex gap-1.5">
            ...
            <button
              ...
              className={cn(
                "p-2.5 rounded-xl transition-all duration-200 active-scale",
                isPrivate
                  ? "bg-gray-200/80 dark:bg-gray-700/60 text-gray-700 dark:text-gray-300"
                  : "text-gray-400 dark:text-gray-500 hover:bg-gray-100/50 dark:hover:bg-gray-700/50"
              )}
              title={isPrivate ? "Private — only you can see this. Click to make public." : "Public — visible to others. Click to make private."}
            >
              {isPrivate ? (
